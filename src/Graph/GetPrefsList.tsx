...
const GetPrefsList = () => {
	...
	const handleClick = (index: number, prefname: string) => {
		...
		.then(data => {
			...
			const currentSeries: Highcharts.SeriesOptionsType = {type: "line",name: prefname, data: currentValues};
				...
				<div className="Prefboxlist">
					{Prefs.map((data: PrefValue) => (
						<div className="Singleset" key={data.prefCode}>
							<input type="checkbox" onClick={() => handleClick(data.prefCode, data.prefName)}/>
